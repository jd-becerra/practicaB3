              ...
              {footer_links.map((link_group, index) => (
                <div key={index}>
                  <button 
                    ...
                    >
                    <h6 className="font-bold text-gray-800 mb-4 ml-2">{link_group.title}</h6>
                  </button>
                  <ul 
                    className={`text-gray-600 text-left pl-4 my-4 bg-white ${toggles[index][0] ? 'block animate-collapse-down' : 'hidden'}`}>
                      {link_group.links.map((link, index) => (
                        <li key={index}><a className='cursor-pointer hover:text-red-600'>{link}</a></li>
                      ))}
                  </ul>
